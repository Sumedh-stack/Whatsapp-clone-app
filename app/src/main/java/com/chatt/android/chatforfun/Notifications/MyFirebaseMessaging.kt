...
class MyFirebaseMessaging : FirebaseMessagingService() {
    override fun onMessageReceived(mRemoteMessage: RemoteMessage) {
        ...
        val sharedRef=getSharedPreferences("PREFS", Context.MODE_PRIVATE)
        val currentonlineUser=sharedRef.getString("currentuser","none")
    ...
    private fun sendNotification(mRemoteMessage: RemoteMessage) {
        ...
        val title=mRemoteMessage.data["user"]
        ...
        val builder= NotificationCompat.Builder(this).setSmallIcon(icon!!.toInt()).setContentTitle(title).setContentText(body).setAutoCancel(true).setSound(defaultSound).setContentIntent(pendingIntent)
